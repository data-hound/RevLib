	...
	while True:
		...
		if len(replymsg) > 0 : 
			...
			for row in cur.execute(q):
				valuereturn = True
				# print(row[8] + "-" + row[12])
				return row[8] + "-" + row[12], handlereply
			# print("> query done")
...
class CookieHandler(object):
	...
	def usage(self) -> str:
		return '''
			   Hi, myself cookie, your shopping assistant. Let me know all the details of smartphones you need and I would suggest you the best ones available at the best price!
			   '''
	def handle_message(self, message: Dict[str, str], bot_handler: Any) -> None:
		if CookieHandler.start == False:
			params = bot_handler.storage.get("p")
			handlereply = bot_handler.storage.get("h")
			replymsg = bot_handler.storage.get("r")
			# print(params)
		else:
			CookieHandler.start = False
		...
		bot_response, handlereply = solve(message['content'], params, handlereply, replymsg)
		bot_handler.storage.put("p", params)
		bot_handler.storage.put( "h" , handlereply)
		bot_handler.storage.put( "r" , replymsg)
		bot_handler.send_reply(message, bot_response)	

handler_class = CookieHandler
